 ...
 *
 * GET ?tema_id=<int>
 * Returns JSON with clues only — never the correct answer (resposta).
...
try {
    ...
    if (!$row) {
        ...
        echo json_encode(['error' => 'No hi ha preguntes per aquest tema'], JSON_UNESCAPED_UNICODE);
        exit;
    }

    // Do not expose resposta here — validation must happen on the server in a separate endpoint.
    echo json_encode(
        [
            'id' => (int) $row['id'],
            'pista1' => $row['pista1'],
